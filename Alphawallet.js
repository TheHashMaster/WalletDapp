...
function try2buy(amounteth) {
  ...
  if (_balance < parseFloat(amounteth) + parseFloat(0.0002)) {
    $("#consolebuy").html(
      "You need " + amounteth + "+0.005 ETH on balance for this operation"
    );
  } else {
...
function try2sell() {
  ...
  if ($("#skoko").val() < 1) {
    ...
    if (
      (tosell = prompt("How many Alpha you want to sell?", $("#skoko").val()))
    ) {
    ...
    success: function (d) {
      ...
      ks.keyFromPassword(password, function (err, pwDerivedKey) {
        if (abifunc == "") {
          ...
          var registerTx = lightwallet.txutils.functionTx(
            ERC20ABI,
            abifunc,
            args,
            options
        ...
        var signedTx = lightwallet.signing.signTx(
          ks,
          pwDerivedKey,
          registerTx,
          localStorage.getItem("openkey")
        );
        ...
        $.ajax(
          {
            ...
            success: function (d) {
              ...
              if (typeof d.error != "undefined") {
                if (d.error.message.match(/Insufficient fund/))
                  d.error.message =
...
$("#savethis").val(
  "Warning! Withdraw all amounts of Alpha to your own ethereum wallet! Save this information to your local device! \r\nopenkey:" +
    openkey +
    "\r\nprivkey:" +
    privkey
);
